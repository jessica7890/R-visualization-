...
######################################
#by default, left open, right closed->change closed="left"
...
#statbin: change the location of the numbers. bar, or the right bar. right:(0,3000]   left:[0,3000)
...
select(tennis,-tourney_id:-loser_rank )#when you want to delete some amount of columns and keep others 
...
#missing value: na=false->included into calculation
...
diamonds %>%
  ...
  arrange(desc(mean.price))%>%#by default, it uses an asending order
  #ungroup()%>% #need to ungroup. delete the group structure.
  #if this is not included, R will find the highest one for
  #each clarity type (for each of the first var).
  top_n(1) #select the top number of rows. 
#what happened if use this right after the above calculation?
