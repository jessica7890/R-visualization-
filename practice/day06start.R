...
ggplot()+
  geom_density(aes(x=carat),size=1,adjust=.5,data=diamonds)
...
ggplot()+
  geom_density(aes(x=petal.length,fill=species),data=iris,alpha=0.5)
...
#ggplot()+ geom_density(aes(x=price), data=diamonds)+ f
#facet_grid()
...
ggplot()+
  geom_boxplot(aes(x=clarity, y=log10(price)), data=iris) 
