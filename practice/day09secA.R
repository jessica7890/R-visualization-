...
ts %>% filter(winner_name==as.character(player_selected)|
...
diamonds %>% 
  ...
  ungroup() %>% 
  top_n(1)#if do not specigy which variable to work with, it will select according to the last.
...
#2. Ohio BRFSS data
##############################################
#The Behavioral Risk Factor Surveillance System (BRFSS) is the nations premier
